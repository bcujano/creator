...
export const FORMATOS = ['pdf', 'docx', 'pptx', 'xlsx', 'md'] as const
...
export async function responderExportacion(doc: Documento, formato: Formato) {
  ...
  if (formato === 'pdf') cuerpo = await (await import('./pdf')).generarPdf(doc)
  ...
  return new NextResponse(typeof cuerpo === 'string' ? cuerpo : new Uint8Array(cuerpo), {
    headers: {
      ...
      'content-disposition': `attachment; filename="${nombreArchivo(doc, formato)}"`,
